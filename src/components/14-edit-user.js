...
const UpdateUser = ({setter})=>{
    ...
    return(
        ...
        <form onSubmit={submitHandler}>
            ...
            <input onChange={(event)=>setPassword(event.target.value)} type="password" minLength="5" placeholder="oldpassword"></input><br></br>

            <label>Please enter new account details:</label><br></br>
            <input onChange={(event)=>setNewUsername(event.target.value)} type="text" maxLength="35" placeholder="Username"></input><br></br>
            
            <input onChange={(event)=>setNewEmail(event.target.value)} type="email" placeholder="Email"></input><br></br>
            <input onChange={(event)=>setNewPassword(event.target.value)} type="password" minLength="5" placeholder="Password"></input><br></br>
